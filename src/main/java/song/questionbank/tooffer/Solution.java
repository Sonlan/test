 ...
 * 消息中间件中有两个概念：消息和队列。
 ...
 随着业务的发展，消息系统需要对队列数进行扩容，为了实现不停机的扩容方式，且保证消息在扩容过程中不会乱序，约定扩容时需要按当前队列的倍数进行扩容。
 因为大多数情况下，消息消费的速度慢于生产的速度，为了保证扩容过程的消息有序性约定：在t时刻新扩容的队列可以立马接收消息，但必须等旧队列上在t时刻前接收到的消息被全部消费完毕后才可以被消费者进行消费。
